...
**Управление сессией** использует HTTP-only куку (`sb_access_token`), установленную бэкендом после
успешного входа. Кука передаётся автоматически с каждым запросом (`withCredentials: true`).
Бэкенд проверяет JWT Supabase локально с помощью `SUPABASE_JWT_SECRET` — без лишних
обращений к Supabase при каждом запросе.
...
| `POST` | `/auth/logout` | Очистка куки сессии |
